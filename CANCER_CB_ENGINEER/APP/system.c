...
/*执行元件初始化1*/  	
	RC_init();
	led_init();
	relay_init();//继电器的初始化放在舵机初始化之前 因为弹仓开合的init保留在舵机的初始化里
  servo_init();
	ducted_init();//涵道 
...
//	static uint32_t loopTime_10ms = 0;  
  static uint32_t loopTime_20ms = 0;  //丢控保护
	...
	static uint32_t loopTime_2000ms = 0; //发消息给队友
...
//			trailer_task();
		...
		if((int32_t)(currentTime - loopTime_2000ms) >= 0)  
		{			
			loopTime_2000ms = currentTime + 2000000;	//2s	
...
//	{			
//		loopTime_500ms = currentTime + 500000;	//500ms	
//		
//		/*给队友发消息*/
//		
//		send_to_teammate();
