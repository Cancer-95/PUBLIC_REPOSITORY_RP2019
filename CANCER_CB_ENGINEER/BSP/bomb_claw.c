...
{	                                                  
	for(u8 k = 0; k < 4; k++)
	{
		...
		if(bomb_claw.zero_correct && (k==0| k==1))//翻转至水平的时候不要堵转保护 因为这个时候的堵转自然会有直接的电流值赋予
		{
			bomb_claw.stall_protect_record[k] = ENABLE;
			bomb_claw.across_stall_protect[k] = DISABLE;
			bomb_claw.stall_times[k] = 0;
			bomb_claw.relax_trigger[k] = 0;
		}
		else
		{
			if((abs(bomb_claw.current[k]) > 4500) || bomb_claw.across_stall_protect[k])
			{
					 ...
					 (millis() - bomb_claw.stall_protect_time[k] < 2080))
				{
					...
					{
						bomb_claw.current[k] = 2000;
					}
					else bomb_claw.current[k] = -2000;
				}
				else if(millis() - bomb_claw.stall_protect_time[k] > 2080)
				{
					...
					bomb_claw.relax_trigger[k] ++;//完成保护一次 放松触发次数++
				}				
			}
			else 
			{
			...
			if(bomb_claw.relax_trigger[k] > 4 && abs(bomb_claw.current[k]) > 6666)
			{
				if(bomb_claw.current[k] > 0)
				{
					bomb_claw.current[k] = 2000;
...
//			      (abs(bomb_claw.target_zero[R] - bomb_claw.total_angle[R]) < 100))		
		else if(bomb_claw.zero_correct)		
		{
			bomb_claw.from_claw_entire[R_axis] = NO;
			bomb_claw.from_claw_bit[R_axis]    = NO;
			bomb_claw.from_claw_half[R_axis]   = NO;
...
/*----------------------------------电机控制----------------------------------*/		
	...
	/*----------------------------------R轴----------------------------------*/		
			...
			case R_CLAW_ENTIRE://爪子完全收回来
			{
				...
				{
				  if(bomb_claw.zero_correct_hold)
					{
						bomb_claw.target_entire[L]     = bomb_claw.zero_stall_point[L] - 13100;
						bomb_claw.target_entire[R]     = bomb_claw.zero_stall_point[R] + 13100;			
					}
					else 
					{
						bomb_claw.target_entire[L]     = bomb_claw.zero_stall_point[L] - 12100;
						bomb_claw.target_entire[R]     = bomb_claw.zero_stall_point[R] + 12100;	
					}
					bomb_claw.zero_correct         = HAVENT;
					bomb_claw.LR_stall_time_record = YES;
			...
			case R_CLAW_HALF://爪子垂直
			{
				if(bomb_claw.flag_claw_half[R_axis])
				{
					if(bomb_claw.zero_correct_hold)
					{
						bomb_claw.target_half[L]       = bomb_claw.zero_stall_point[L] - 7100;
						bomb_claw.target_half[R]       = bomb_claw.zero_stall_point[R] + 7100;			
					}
					else 
					{
						bomb_claw.target_half[L]       = bomb_claw.zero_stall_point[L] - 6100;
						bomb_claw.target_half[R]       = bomb_claw.zero_stall_point[R] + 6100;			
					}
					bomb_claw.zero_correct         = HAVENT;
					bomb_claw.LR_stall_time_record = YES;
				...
				{
					bomb_claw.target_angle[L] = bomb_claw.total_angle[L] + 1000;					
				}			
				...
				{
					bomb_claw.target_angle[R] = bomb_claw.total_angle[R] - 1000;					
				}			
			...
			{				
				if(bomb_claw.flag_claw_bit[R_axis])
				{
					if(bomb_claw.zero_correct_hold)
					{
						bomb_claw.target_bit[L]        = bomb_claw.zero_stall_point[L] - 12500;
						bomb_claw.target_bit[R]        = bomb_claw.zero_stall_point[R] + 12500;				
					}
					else 
					{
						bomb_claw.target_bit[L]        = bomb_claw.zero_stall_point[L] - 11500;
						bomb_claw.target_bit[R]        = bomb_claw.zero_stall_point[R] + 11500;					
					}
					bomb_claw.zero_correct         = HAVENT;
					bomb_claw.LR_stall_time_record = YES;
			...
			{							
				if(bomb_claw.flag_claw_zero[R_axis])
				{
					...
					bomb_claw.zero_brake_point[L] = bomb_claw.total_angle[L] + 5555;
					bomb_claw.zero_brake_point[R] = bomb_claw.total_angle[R] - 5555;
					/*标志位处理*/
				...
				{
					if(bomb_claw.target_angle[L] < bomb_claw.zero_brake_point[L])
					{
						bomb_claw.target_angle[L] = bomb_claw.total_angle[L] + 800;		
					}
					else bomb_claw.target_angle[L] = bomb_claw.total_angle[L] + 222;				
				}
				...
				{
					if(bomb_claw.target_angle[R] > bomb_claw.zero_brake_point[R])
					{
						bomb_claw.target_angle[R] = bomb_claw.total_angle[R] - 800;
					}
					else bomb_claw.target_angle[R] = bomb_claw.total_angle[R] - 222;	
				}
				else bomb_claw.target_angle[R] = bomb_claw.target_zero[R];
					
				if((abs(bomb_claw.speed_fdb[L]) < 50) && 
					 (abs(bomb_claw.speed_fdb[R]) < 50) && 
					 (abs(bomb_claw.current[L])   > 3333) && 
					 (abs(bomb_claw.current[R])   > 3333))
				{
					...
					else if(millis() - bomb_claw.LR_stall_time > 50)
					{
						bomb_claw.zero_stall_point[L] = bomb_claw.total_angle[L];
						bomb_claw.zero_stall_point[R] = bomb_claw.total_angle[R];
						bomb_claw.zero_correct        = FINISHED;
						bomb_claw.zero_correct_hold   = YES;
					}		
				}
...
//		}	
		...
		/*给失控保护*/
		{
			...
			{
				bomb_claw_cas_ctrl(bomb_claw.target_angle);//串级计算
				if(bomb_claw.zero_correct == FINISHED)
				{
					bomb_claw.current[L] =  333;
					bomb_claw.current[R] = -333;
				}
			}
		}	
	}	
