...
{	
  ...
  /*复位速度初始化*/
	...
	lifter.target_tube_bit[L] = -1555;
	lifter.target_tube_bit[R] =  1555;	
  ...
  /*用来调整上岛时摩擦地面程度*/	
	lifter.target_zero_climb_up[L] = -185;
	lifter.target_zero_climb_up[R] =  185;	
	...
	for(u8 k = 0; k < 2; k++)
	{
		...
		if((abs(lifter.current[k]) > 7000) || lifter.across_stall_protect[k])
		{
				 ...
				 (millis() - lifter.stall_protect_time[k] < 2080))
			{
				...
				{
				  lifter.current[k] = 4000;
				}
				else lifter.current[k] = -4000;
			}
			else if(millis() - lifter.stall_protect_time[k] > 2080)
			{
		  	lifter.stall_protect_record[k] = ENABLE;
				...
				if(lifter.ctrl_mode == LIFTER_ZERO)//框架回零点的时候启动放松程序
				{
					lifter.relax_trigger[k] ++;//完成保护一次 放松触发次数++
				}
				else lifter.relax_trigger[k] = 0;//完成保护一次 放松触发次数++
			}				
		}
		else 
		{
			lifter.stall_protect_record[k] = ENABLE;
			lifter.across_stall_protect[k] = DISABLE;
			lifter.relax_trigger[k] = 0;
		}
		
		if(lifter.relax_trigger[k] > 6 && abs(lifter.current[k]) > 7000)
		{
			if(lifter.current[k] > 0)
			{
				lifter.current[k] = 4000;
