...
static void move_direction_ctrl(bool forward, bool back, bool left, bool right)
{
  if (forward)//前进
  {		
		kb.vx = kb.x_spd_limit * ramp_kb_calc(&fb_ramp, CNT_DOWN, kb.acc_time/INFO_GET_PERIOD);
  }
  else if (back)//后退
  {
		kb.vx = kb.x_spd_limit * ramp_kb_calc(&fb_ramp, CNT_UP, kb.acc_time/INFO_GET_PERIOD);			
  }
  else//x方向刹车
  {		
		kb.vx = kb.x_spd_limit * ramp_kb_calc(&fb_ramp, TO_ZERO, kb.acc_time/INFO_GET_PERIOD);
	}
	
  if (left)//左平移
  {
		kb.vy = kb.y_spd_limit * ramp_kb_calc(&lr_ramp, CNT_UP, kb.acc_time/INFO_GET_PERIOD);
  }
  else if (right)//右平移
  {
  	kb.vy = kb.y_spd_limit * ramp_kb_calc(&lr_ramp, CNT_DOWN, kb.acc_time/INFO_GET_PERIOD);
  }
  else//y方向刹车
  {
  	kb.vy = kb.y_spd_limit * ramp_kb_calc(&lr_ramp, TO_ZERO, kb.acc_time/INFO_GET_PERIOD);
  }
