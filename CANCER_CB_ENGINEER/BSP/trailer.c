...
void trailer_param_init(void)
{
  ...
  trailer.ecd_record = ENABLE;
	/*位置初始化*/
	trailer.target_entire = 15000;
	trailer.target_zero = 800;
...
{	
	...
	trailer.current = pidckz_calc(&pid_trailer_speed, trailer.speed_fdb, trailer.speed_ref);            //PID计算
		...
		if((abs(trailer.current) > 4500) || trailer.across_stall_protect)
		{
			...
			if((millis() - trailer.stall_protect_time > 2000) &&
				 (millis() - trailer.stall_protect_time < 2080))
			{
				trailer.across_stall_protect = ENABLE;
				
				if(trailer.current > 0)
				{
				  trailer.current = 3000;
				}
				else trailer.current = -3000;
			}
			else if(millis() - trailer.stall_protect_time > 2080)
			{
				trailer.stall_protect_record = ENABLE;
				trailer.across_stall_protect = DISABLE;
				trailer.stall_times ++;//完成保护一次 堵转次数++
				trailer.relax_trigger ++;//完成保护一次 放松触发次数++
			}	
		}
	  else
		{
			trailer.stall_protect_record = ENABLE;
			trailer.across_stall_protect = DISABLE;
			trailer.relax_trigger = 0;
		}			
		
		if(trailer.relax_trigger > 4 && abs(trailer.current) > 6666)
		{
			if(trailer.current > 0)
			{
				trailer.current = 3333;
			}
			else trailer.current = -3333;
		}
