...
void UART5_IRQHandler(void)
{
	...
	{		
		...
		memset(JudgementRxBuffer + JudgementDataLength, 0, DMA_GetCurrDataCounter(DMA1_Stream0));		
		...
		else 
		{
		  if(data_for_client.preemption_time_record && data_for_client.suspend_flag)
			...
			else
			{
				...
				{
					if(TeammateData.dataFrameHeader.data_cmd_id == 0x0299   && 
						 TeammateData.dataFrameHeader.send_ID     == Sentry   && 
				   	 TeammateData.dataFrameHeader.receiver_ID == Engineer &&
						 TeammateData.interactData.data_frame[0]  == 1        && 
					   data_for_client.execute_task < 10)
					{
