...
{	
  ...
  /*用来调整上岛时摩擦地面程度*/	
	lifter.target_zero_climb_up[L] = -350;
	lifter.target_zero_climb_up[R] = 350;	
...
void lifter_task(void)
{
	if(lifter.reset)//抬升复位完后 抬上来一点给取弹复位
	{
      ...
      lifter.ctrl_mode = BOMB_LIFTER_BIT;
			if((abs(lifter.target_bomb_bit[L] - lifter.total_angle[L]) < 100) || (abs(lifter.target_bomb_bit[R] - lifter.total_angle[R]) < 100))//抬升到目标角度
			{
