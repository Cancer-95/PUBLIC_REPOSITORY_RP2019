	...
	if(climb_tube.climb_enable == ENABLE)	
	{
		switch(climb_tube.ctrl_mode)
		{
			case PLAIN_MODE://平原模式
			{
				...
				if(chassis.ass_mode_enable)GIM_SERVO_BACK;	
				else GIM_SERVO_ZERO;
...
//						ducted.ctrl_mode = DUCTED_R_RUN;                           //右涵道吹
					...
					else if((abs(climb_tube.twisted_angle - 41) < 1) && climb_tube.fiag_launch == 1 && 
						      optic_switch.value[S] == 1)// && optic_switch.value[R] == 1)//陀螺仪转到了位置
					{
						climb_tube.twisted_angle = 41;                               //强制让它到了这个角度
...
//					  TRAILER_HOOK_PULL;   
					...
					if(lifter.total_angle[R] > lifter.target_bomb_bit[R] || lifter.total_angle[L] < lifter.target_bomb_bit[L])
					{
				  	GIM_SERVO_SIDE;
					}
...
//						lifter.ctrl_mode = TUBE_LIFTER_BIT;                          //框架抬升
						...
						chassis.ctrl_mode = CHASSIS_SEPARATE;                      //底盘机械模式
						if((abs(lifter.target_tube_entire[L] - lifter.total_angle[L]) < 120) || (abs(lifter.target_tube_entire[R] - lifter.total_angle[R]) < 120))//抬升到目标角度
						{
              TUBE_CLAW_LOOSE;                                              //爪子松开
//							TUBE_BRAKE_PULL;									                            //刹车收回
							chassis.ctrl_mode = MOVE_FORWARD;                             //底盘向前跑
