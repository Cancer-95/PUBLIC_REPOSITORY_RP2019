...
{		
  ...
  /*复位速度初始化*/
	bomb_claw.reset_speed[L] = 2000;
	bomb_claw.reset_speed[R] = -2000;
...
//	bomb_claw.flag_working_mode = ENABLE;
  ...
  memset(bomb_claw.ecd_Record, ENABLE, sizeof(bomb_claw.ecd_Record));
	...
	bomb_claw.target_entire[L] = -17200;
	bomb_claw.target_bit[L] = -16500;
	bomb_claw.target_half[L] = -9000;	
	bomb_claw.target_zero[L] = -800;
 
	bomb_claw.target_entire[R] =  17200;
  bomb_claw.target_bit[R] = 16500;
	bomb_claw.target_half[R] =  9000;
	bomb_claw.target_zero[R] = 800;
...
{	                                                  
	for(u8 k = 0; k < 4; k++)
	{
		...
		if(abs(bomb_claw.current[k]) > 7000)
		{
			...
			if(millis() - bomb_claw.stall_protect_time[k] > 888)
			{
				if(bomb_claw.current[k] > 0)
				{
				  bomb_claw.current[k] = 3333;
				}
				else bomb_claw.current[k] = -3333;
			}	     	
		}
	  else bomb_claw.stall_protect_record[k] = ENABLE;
	}
}
...
/*----------------------------------电机控制----------------------------------*/		
	...
	/*----------------------------------R轴----------------------------------*/		
			...
			{				
				...
				{
					bomb_claw.target_angle[L] = bomb_claw.total_angle[L] + 700;					
				}			
				...
				{
					bomb_claw.target_angle[R] = bomb_claw.total_angle[R] - 700;					
				}			
		...
		{		
			...
			{									
				...
				if(bomb_claw.target_angle[Y] < bomb_claw.target_entire[Y])
				{
					...
					else bomb_claw.target_angle[Y] = bomb_claw.total_angle[Y] + 3000;//取弹速度	
				}
			...
			case Y_CLAW_HALF://爪子中间
			{
				...
				{
					bomb_claw.target_angle[Y] = bomb_claw.total_angle[Y] - 3000;					
				}			
...
void bomb_claw_reset(void)
{
	...
	if(!bomb_claw.LR_reset)
	{
		   ...
		   (abs(bomb_claw.current[R]) > 5000))
		{
			...
			if(millis() - bomb_claw.LR_stall_time > 200)
			{
