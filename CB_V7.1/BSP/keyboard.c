...
static void move_spd_ctrl(uint8_t fast, uint8_t slow)
{
  ...
  else
  {
    ...
    kb.x_spd_limit = 0.9f * CHASSIS_KB_MAX_SPEED_X;
    kb.y_spd_limit = 0.9f * CHASSIS_KB_MAX_SPEED_Y;
		kb.w_spd_limit = 0.9f * CHASSIS_KB_MAX_SPEED_R;
  }
}
/*底盘方向控制*/
static void move_direction_ctrl(bool forward, bool back, bool left, bool right)
{
  ...
  else//y方向刹车
  {
		 ...
		 flag_gyro_blow_up)//底盘是机械模式或者标记了陀螺仪爆炸
	{
		...
		{		  
      if(chassis.ctrl_mode == VISION_FEED_BOMB)//补弹模式下还得更慢
			{
			  kb.vw = -RC_Ctl.mouse.x * 0.8;
			}		
...
{ 
  ...
  { 
		...
		/*原地打转*/
		if((chassis.ctrl_mode == CHASSIS_SEPARATE) || chassis.ctrl_mode == CHASSIS_FOLLOW)//底盘可控的情况下才使能SPIN模式
		{
			kb_spin_around_ctrl(SPIN_AROUND);
		}
		else kb_spin_around_ctrl(SPIN_AROUND_DISABLE);
  }
  else
  {
    kb.vx = 0;
...
///*辅助支撑腿的控制*/
//bool flag_supporting_wheel = 0;
//static void kb_supporting_wheel_ctrl(uint8_t supporting_wheel_ctrl) 
//{
//	static bool flag_toggle = 1;
//	
//	if(supporting_wheel_ctrl && flag_toggle)
//	{
//		flag_supporting_wheel = !flag_supporting_wheel;
//		flag_toggle = 0;
//	}
//	else if(!supporting_wheel_ctrl) flag_toggle = 1;

//	if(flag_supporting_wheel)//根据flag判断要做什么
//	{
//		SUPPORTING_WHEEL_PUSH;
//	}		
//	else SUPPORTING_WHEEL_PULL;
//}
