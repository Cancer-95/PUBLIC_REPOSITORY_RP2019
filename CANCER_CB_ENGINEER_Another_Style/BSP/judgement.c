...
unsigned char _UART5_DMA_RX_BUF[UART5_DMA_RX_BUF_LEN];

unsigned char _CLIENT_TX_BUF[UART5_TX_BUF_LEN];

unsigned char _TEAMMATE_TX_BUF[UART5_TX_BUF_LEN];
...
//		DMA_InitStructure.DMA_Memory0BaseAddr = (uint32_t)_UART5_DMA_RX_BUF;
//		DMA_InitStructure.DMA_DIR = DMA_DIR_PeripheralToMemory;
//		DMA_InitStructure.DMA_BufferSize = UART5_DMA_RX_BUF_LEN;//接受区的大小
...
//	}
  ...
  data_for_client.status_bar_record[SENTRY_MAYDAY] = ENABLE;
	data_for_client.status_bar_record[RESETING] = ENABLE;
	data_for_client.status_bar_record[MOTOR_OFFLINE] = ENABLE;
	data_for_client.preemption_time_record = ENABLE;
}
...
void receive_data_from_judgement(unsigned char *JudgementData)
{
	...
	&& (Verify_CRC16_Check_Sum(JudgementData, sizeof(extFrameHeader) + CMD_ID_SIZE + FrameHeader.DataLength + FRAME_TAIL_SIZE) == 1))
	{
		...
		switch (CmdID)
		{
			...
			{
				memcpy(&TeammateData, JudgementData + 7, sizeof(TeammateData));
				break;
			}
		}
...
{
	static uint8_t UART5_DataLength = 0;
	...
	{		
		...
		UART5_DataLength = UART5_DMA_RX_BUF_LEN - DMA_GetCurrDataCounter(DMA1_Stream0);
		
		receive_data_from_judgement(_UART5_DMA_RX_BUF);

		memset(_UART5_DMA_RX_BUF + UART5_DataLength, 0, DMA_GetCurrDataCounter(DMA1_Stream0));		
...
void send_to_client(void)
{
	...
	memcpy(_CLIENT_TX_BUF, 
	       &SendClient.txFrameHeader, 
	       sizeof(extFrameHeader));
	Append_CRC8_Check_Sum(_CLIENT_TX_BUF, sizeof(extFrameHeader));	
...
void send_to_teammate(void)
{
	...
	memcpy(_TEAMMATE_TX_BUF, 
	       &CommuData.txFrameHeader, 
	       sizeof(extFrameHeader));
  Append_CRC8_Check_Sum(_TEAMMATE_TX_BUF, sizeof(extFrameHeader));	
	
	CommuData.CmdID = 0x0301;

	CommuData.dataFrameHeader.data_cmd_id = GOLF_NUM_FOR_HERO;//
	
	CommuData.dataFrameHeader.send_ID = Engineer;//发送者的ID
	CommuData.dataFrameHeader.receiver_ID = Hero;//接受者的ID
	
	CommuData.interactData.data_frame[0] = (uint8_t)data_for_client.golf_num;//大小不要超过变量的变量类型 (发送弹量)  

	memcpy(_TEAMMATE_TX_BUF + 5,
	       (uint8_t *)&CommuData.CmdID,
	       (sizeof(CommuData.CmdID)+
					sizeof(CommuData.dataFrameHeader) +
					sizeof(CommuData.interactData)));		
	Append_CRC16_Check_Sum(_TEAMMATE_TX_BUF,sizeof(CommuData));
	
	datalength = sizeof(CommuData); 
	for(i = 0;i < datalength;i++)
	{
		USART_SendData(UART5,(uint16_t)_TEAMMATE_TX_BUF[i]);
		while(USART_GetFlagStatus(UART5,USART_FLAG_TC)==RESET);
	}	 
}
...
{
	static u8 i,u;//这两个变量收到哨兵求救后 用来更迭灯条 i用来计时 u用来滚动
  ...
  /*-------------------------取多少箱&位置提示-------------------------*/
	...
	/*-------------------------状态条提示-------------------------*/
	{
    if(((CAN1_motor_offline_check() || CAN2_motor_offline_check() || check.motor_serious_stall == 1) 
			  && !data_for_client.suspend_flag) || data_for_client.timing_flag) //电机离线或者严重堵转 此优先级最高 设计为强制抢占灯条10s 全红全绿交替闪烁
		{	
			/*------------------亮灯程序------------------*/
			if(data_for_client.status_bar_record[MOTOR_OFFLINE])
			{
				i = 0;
				data_for_client.status_bar = ALL_RED; //状态条
				data_for_client.status_bar_record[MOTOR_OFFLINE] = DISABLE;
			}
			else if(i > 5)
			{
				data_for_client.status_bar = data_for_client.status_bar ^ ALL_GREEN;
				i = 0;
			}	
			else i++;
		/*-----------------------------------------------*/
		  if(data_for_client.preemption_time_record)
			{
				data_for_client.preemption_time_record = DISABLE;
				data_for_client.timing_flag = ENABLE;
				data_for_client.preemption_time = millis();
			}		
			else if(millis() - data_for_client.preemption_time > 8000)//午时已到 悬起来
			{
				data_for_client.suspend_flag = ENABLE;
				data_for_client.timing_flag = DISABLE;
				data_for_client.preemption_time_record = ENABLE;
			}
		}
		else 
		{
		  if(data_for_client.preemption_time_record && data_for_client.suspend_flag)
			{
				data_for_client.preemption_time_record = DISABLE;
				data_for_client.timing_flag = DISABLE;
				data_for_client.preemption_time = millis();
			}
			else if(millis() - data_for_client.preemption_time > 8000)//午时已到 放开它
			{
				data_for_client.suspend_flag = DISABLE;
				data_for_client.preemption_time_record = ENABLE;
			}
			
			
			if((!MOTORS_ALL_READY) && RC_IS_READY)//在复位的时候 灯条滚动 滚完了再执行常规模式
			{
				if(data_for_client.status_bar_record[RESETING])
				{
					i = 0;
					u = 0;
					data_for_client.status_bar = ALL_RED; //状态条
					data_for_client.status_bar_record[RESETING] = DISABLE;
				}
				else if(i > 2)
				{
					data_for_client.status_bar = data_for_client.status_bar ^ (1 << u);
					u > 4 ? u = 0 : u ++ ;
					i = 0;
				}	
				else i++;
			...
			{
				if(vision.enable || chassis.ctrl_mode == MOVE_FORWARD)//视觉模式以及爬杆后的自动底盘移动
				{
					data_for_client.status_bar_record[SENTRY_MAYDAY] = ENABLE;
					
					if(vision.captured == 9)//视觉摄像头找不到了
					{
				  	data_for_client.status_bar = CRISS_CROSS;//状态条交错
					}
					else data_for_client.status_bar = ALL_GREEN;//状态条全绿

				}
				else if(flag_bomb_claw_scan)//扫描模式
				{
					if(get_bomb.flag_scan_failed)//扫描失败
					{
						data_for_client.status_bar = CRISS_CROSS;//状态条交错
						data_for_client.status_bar_record[SENTRY_MAYDAY] = ENABLE;
					}
					...
					{
						data_for_client.status_bar = HALF_HALF;  //状态条一半一半
						data_for_client.status_bar_record[SENTRY_MAYDAY] = ENABLE;
					}
				}
				else //不在强制工作的时候才能接收哨兵的Mayday
				{
					if((TeammateData.interactData.data_frame[0] == 0x01) && data_for_client.execute_task < 10)
					{
						if(data_for_client.status_bar_record[SENTRY_MAYDAY])
						{
							i = 0;
							data_for_client.status_bar = ALL_RED; //状态条
							data_for_client.status_bar_record[SENTRY_MAYDAY] = DISABLE;
						}
						else if(i > 6)
						{
							data_for_client.status_bar = data_for_client.status_bar ^ 1;
							i = 0;
						}	
						else i++;
					}
					...
					{
						data_for_client.status_bar = ALL_RED; //状态条全红
						data_for_client.status_bar_record[SENTRY_MAYDAY] = ENABLE;
					}	
				}			
				
				if(flag_magazine_exit || rescue.ctrl_mode == HOOK_CONFIRM_MODE)//补弹中或者是确认钩住模式
				{
					data_for_client.status_bar = CRISS_CROSS;//状态条交错
					data_for_client.status_bar_record[SENTRY_MAYDAY] = ENABLE;
				}
				if(flag_trailer_hook)//拖车那个钩子打下啦
				{
				  data_for_client.status_bar = ALL_GREEN;
					data_for_client.status_bar_record[SENTRY_MAYDAY] = ENABLE;
				}
