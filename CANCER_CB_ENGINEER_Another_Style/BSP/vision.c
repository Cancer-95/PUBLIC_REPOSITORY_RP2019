...
//接收到的数据暂存在这里
unsigned char _UART4_DMA_RX_BUF[UART4_DMA_RX_BUF_LEN] = {0};
...
//	DMA_InitStructure.DMA_Memory0BaseAddr     = (uint32_t)_UART4_DMA_RX_BUF;
//	DMA_InitStructure.DMA_DIR = DMA_DIR_PeripheralToMemory;
//	DMA_InitStructure.DMA_BufferSize = UART4_DMA_RX_BUF_LEN;
...
void UART4_IRQHandler(void)
{
  ...
  if(USART_GetITStatus(UART4,USART_IT_IDLE)!=RESET)//检测到空闲线路
	{		
		...
		UART4_Datalength = UART4_DMA_RX_BUF_LEN - DMA_GetCurrDataCounter(DMA1_Stream2);

		/*读取与赋值*/
		uart4_read_data(_UART4_DMA_RX_BUF);//读取缓存区数据	
		...
		memset(_UART4_DMA_RX_BUF + UART4_Datalength, 0, DMA1_Stream2 -> NDTR);		//后面的全清零//重新覆盖接收数组//数组首地址
 		DMA1_Stream2 -> NDTR = UART4_DMA_RX_BUF_LEN;
...
void uart4_send_data( uint8_t CmdID, uint8_t data_0)
{
	...
	{
	  USART_SendData(UART4,uart4_send_pack[i]); 		
		while(USART_GetFlagStatus(UART4,USART_FLAG_TC)==RESET); 
	}
}
...
void vision_task(void)
{
	if(vision.enable)
	{			
//		chassis.ass_mode_enable = ENABLE;        //除了爬杆其他均开启车尾模式
		
		if(climb_tube.location == ON_GROUND)
		{
			if(climb_tube.vision_enable)//爬杆的时候自动对杆子
			{
				pidckz_param_set(&pid_vision_chassis_vy, 500, 100, 2.2f, 0.0001f, 0.0f);   
        vision.y_ref = 700;	
				LED_CAMERA_ON;
				vision.TX_Cmd = ENABLE_CLIMB_TUBE_VISION;
			}
//			else if(feed_bomb.ctrl_mode == AIM_FEED_MODE)
			else if(feed_bomb.vision_enable)
			{
			  pidckz_param_set(&pid_vision_chassis_vy, 500, 100, 5.5f, 0.002f, 10.0f);   
        vision.y_ref = 72;	
				LED_CAMERA_ON;
				vision.TX_Cmd = ENABLE_FEED_BOMB_VISION;
			}
			else
			{
				pidckz_param_set(&pid_vision_chassis_vy, 500, 100, 3.0f, 0.005f, 5.0f);   
				vision.y_ref = 666;	
				LED_CAMERA_ON;
				vision.TX_Cmd = ENABLE_GROUND_GET_BOMB_VISION;
		...
		else if(climb_tube.location == ON_ISLAND)//岛上就只允许了取弹视觉
		{
//			if(feed_bomb.ctrl_mode != AIM_FEED_MODE)//岛上在补弹对准的情况下按下shift+V是没有用的
//			{
				pidckz_param_set(&pid_vision_chassis_vy, 500, 100, 2.5f, 0.005f, 2.0f);   
				vision.y_ref = 650;	
				LED_CAMERA_ON;
				vision.TX_Cmd = ENABLE_ISLAND_GET_BOMB_VISION;
...
//			}
    ...
    if(get_bomb.ctrl_mode != PUSH_AIM_SCAN_MODE)//在扫描模式下就不要乱动框架值了
		...
		vision.TX_Cmd = DISABLE_ALL_VISION;//不要给我发东西了
