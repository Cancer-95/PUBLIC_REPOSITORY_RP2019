...
/**
	* @brief      临界值处理
	* @param[in]  range: 编码器(陀螺仪)总范围 即首尾读数之差的绝对值 如0~360或-180~180 range都为Ϊ360
	* @param[in]  mid: 范围数值零点 如0~360则mid为180 如-180~180则mid为0
	* @note       以下是改进的临界值处理函数 增加了一个入口函数
	*/
...
void moving_average_filter(float data_in, uint8_t num, float *data_out)//滑动平均值滤波器(num:取样个数)
{
	uint8_t i;
	float buf[num+1];
	float sum = 0;
...
/**
	* @brief      各类型的限幅：float、int32_t、int16_t、int
	* @note       
	*/
